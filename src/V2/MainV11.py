...
model_path = '/data/s4120310/ModelsV11/'
history_path = '/data/s4120310/HistoryV11/'
...
class CarlaManager(object):
    ...
    def train(self,save_path,hist_path,iteration ,epochs = 500,batch_size = 200,freq_decrease = 1000):
        ...
        for epoch in range(epochs):
            ...
            try:
                ...
                frame = self.world.tick()
                cmd,reward,dp,_ = self.scenario.step(self.ego_vehicle)
                frame = self.world.tick()
            except Exception as e:
            ...
            lock = 0
            if victory_buffer >= 100000:
                section+=1
                victory_buffer = 0
            print("Begin")
            while not done:
                speed = self.scenario._veh.speed/1.99
                way = self.scenario._target_lane_waypoint.transform
                way.location.x = self.ego_vehicle.get_location().x + 40
                ...
                if step < prep_limit:
                    ...
                    try:
                        ...
                        data = self.prep_neighbours(_["ngsim_dataset"]["frame"])
                        data = data + inps
                        #inps = np.array(inps)
    ...
    def validate(self):
        ...
        self.scenario.reset(self.ego_vehicle,0)
        self.world.tick()
        val_count = 0
        for i in range(100):
            t_clip_n = 0.0
            ...
            #    crop_type=BirdViewCropType.FRONT_AREA_ONLY
            #    )
            self.scenario.reset(self.ego_vehicle,i)
            c = self.world.tick()
            self.ego_vehicle.apply_control(carla.VehicleControl(manual_gear_shift=True, gear=4))
            c = self.world.tick()
            cmd,reward,dp,_ = self.scenario.step(self.ego_vehicle)
