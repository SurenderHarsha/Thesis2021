...
model_path = '/data/s4120310/ModelsV3/'
history_path = '/data/s4120310/HistoryV3/'
...
class CarlaManager(object):
    ...
    def __init__(self,random_seed = 2,host = 'localhost',port = 2000):
    ...
    def train(self,save_path,hist_path,iteration ,epochs = 500,batch_size = 200,freq_decrease = 1000):
        ...
        for epoch in range(epochs):
            ...
            while not done:
                ...
                if len(cmd_buffer) > 5:
                    if sum(cmd_buffer[-5:]) == 0 and _['on_target_lane']:
                        reward = 1.1
                        done = True
                way = self.way_cal(self.ego_vehicle,val)
